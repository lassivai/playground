...
void main()
{
  ivec2 isize = imageSize(hitCounterImageArray).xy;
  ...
  for(int i=0; i<maxIter; i++) {
    for(int n=0; n<10; n++) {
      ...
      switch(variations[n])
      {
        ...
        case 15: { // waves
          ...
          float c = max(parameters[n*14 + 8], 1e-8);
          float f = max(parameters[n*14 + 9], 1e-8);
        ...
        case 65: { //
          ...
          float u = parameters[n*14 + 8] * 0.5;
          float s1 = 0, s2 = 0;
          //while((s1 = randomInteger(q, -t, t)) == 0) {}
          s1 = randomInteger(q, -t, t+1);
          //s1 = randomInteger(q, 1, t);
          //while((s2 = Random::getInt(-t, t)) == 0) {}
          float k1 = s1 * PI / (1.0+t);
          float k2 = s1 * PI / (1.0+t);
          w += vec2(sr*cos(p*u + k1), sr*sin(p*u + k2));
        } break;
        ...
        case 66: { // julia
          float sr = sqrt(length(q));
          float p = atan(q.y, q.x);
          float k = randomMinusOneOrOne(q) * PI * 0.5 * parameters[n*14 + 8];
          float t = parameters[n*14 + 7] * 0.5;
          w += vec2(sr*cos(p*t + k), sr*sin(p*t + k));
        } break;
    ...
    if(i >= minIter) {
      ...
      if(px >= 0 && px < isize.x) {
        ...
        if(py >= 0 && py < isize.y) {
          imageAtomicAdd(hitCounterImageArray, ivec3(px, py, activeHitCounterImageIndex), 1);
        }
      }
    }
