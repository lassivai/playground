...
vec2 random2(in vec2 st) {
    return vec2(fract(sin(dot(st.xy, vec2(12.9898,78.233))) * 43758.5453123),
                fract(sin(dot(st.xy, vec2(62.6436,26.5256))) * 726432.7653234));
}
...
vec2 fbm2(vec2 st, float t = 0, vec2 value = vec2(0.0), float frequency = 1.0,
          float amplitude = 0.5, float gain = 0.5, float lacunarity = 2.0) {

  for(int i=0; i<OCTAVES; i++) {
    value.x += amplitude * noise(st * frequency + t);
    value.y += amplitude * noise(vec2(1455.43, 4253.43) + st * frequency + t);
    frequency *= lacunarity;
    amplitude *= gain;
  }
...
float random(int i) {
  return random(vec2(sin(float(i)*37542.73)*35.3565, sin(float(i)*46756.273)*15.54));
}
...
float pat(vec2 p, vec2 size, float t) {
  ...
  float d = 0.;
  float a = atan(p.y-size.y*0.5, p.x-size.x*0.5) * 2.0;
  return 1.0-smoothstep(.0, 1.0, fract(a / TWO_PI + d / size.x * 40.0 / TWO_PI + t));
...
void main()
{
   int numCells = int(size.y*0.3);
   vec2 pos = texCoord * float(numCells);
   pos.x *= size.x/size.y;
   //pos.x *= size.x/size.y;
   //pos = pos*2.0 - vec2(1.0, 1.0);
   pos.x += time * 10.0;

   vec2 m = mouse / size * numCells ;
   m.x = m.x * size.x/size.y + time * 10;

   //float o = time * 2.0;
   //vec2 lightDir = vec2(cos(o), sin(o));
   ...
   //colorOut = c * isPrime(ipos.x ^ ipos.y);
   ...
   for(int i=-2; i<3; i++) {
     for(int j=-2; j<3; j++) {
       //vec2 p = random2(vec2(ipos + ivec2(i, j))) + vec2(float(i), float(j));
       vec2 p = 2.*fbm2(vec2(ipos + ivec2(i, j))*100.0, time*0.25) + vec2(float(i), float(j));
       vec2 center = vec2(vec2(0.5) + vec2(i, j));
       vec2 p2 = normalize(center+ipos - m);
       p = mix(p, p-p2, min(1.0, 5./distance(center+ipos, m)));
       float d = distance(p, fpos);
       int index = ipos.x + i + (ipos.y+j) * numCells;
       if(d < minDist) {
         minDist2 = minDist;
         minIndex2 = minIndex;
         minP2 = minP;
   ...
   float pr = isPrime(ipos2.x ^ ipos2.y);

   //vec3 c = random3(minIndex);
   vec3 c = vec3(1.0);

   float t = (minDist2 - minDist) / distance(minP, minP2);

   //float w = pat(vec2(ipos)+minP, vec2(numCells, numCells), time*.5);
   float w = 1.;

   vec2 v = minP2 - minP;
   float a = angle(v, pos-m) / PI / (0.1*distance(pos, m));

   //float levels = 5;
   //t = int(t*levels) / levels;

   colorOut = vec4(vec3(c*a*pr), 1.0) * w;

/*
   colorOut = max(colorOut,
     vec4(1.0) * max(step(0.99, fract(pos.x)), step(0.99, fract(pos.y))));

   colorOut = max(colorOut,
     vec4(1.0) * (1.0-step(0.03, distance(vec2(ipos) + minP, pos))));
*/
