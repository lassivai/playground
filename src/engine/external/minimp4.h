...
enum
{
    ...
    BOX_calb    = FOUR_CHAR_INT( 169, 'a', 'l', 'b'),    // album
    BOX_cart    = FOUR_CHAR_INT( 169, 'a', 'r', 't'),    // artist
    BOX_aART    = FOUR_CHAR_INT( 'a', 'A', 'R', 'T' ),      // album artist
    BOX_ccmt    = FOUR_CHAR_INT( 169, 'c', 'm', 't'),    // comment
    BOX_cday    = FOUR_CHAR_INT( 169, 'd', 'a', 'y'),    // year (as string)
    BOX_cnam    = FOUR_CHAR_INT( 169, 'n', 'a', 'm'),    // title
    BOX_cgen    = FOUR_CHAR_INT( 169, 'g', 'e', 'n'),    // custom genre (as string or as byte!)
    BOX_trkn    = FOUR_CHAR_INT( 't', 'r', 'k', 'n'),       // track number (byte)
    BOX_disk    = FOUR_CHAR_INT( 'd', 'i', 's', 'k'),       // disk number (byte)
    BOX_cwrt    = FOUR_CHAR_INT( 169, 'w', 'r', 't'),    // composer
    BOX_ctoo    = FOUR_CHAR_INT( 169, 't', 'o', 'o'),    // encoder
    ...
    BOX_cgrp    = FOUR_CHAR_INT( 169, 'g', 'r', 'p'),    // grouping
    ...
    BOX_clyr    = FOUR_CHAR_INT( 169, 'l', 'y', 'r'),    // lyrics (may be > 255 bytes)
...
int MP4E__set_dsi(MP4E_mux_t *mux, int track_id, const void *dsi, int bytes)
{
    ...
    assert(tr->info.track_media_kind == MP4E_track_t::e_audio ||
           tr->info.track_media_kind == MP4E_track_t::e_private);
    if (tr->vsps.bytes)
    {
...
{
    track_t* tr = ((track_t*)mux->tracks.data) + track_id;
    assert(tr->info.track_media_kind == MP4E_track_t::e_video);
...
#define MOOV_TIMESCALE 1000
    ...
    for (ntr = 0; ntr < ntracks; ntr++)
    {
        ...
        ATOM(BOX_trak);
            ...
            if (tr->info.track_media_kind == MP4E_track_t::e_audio || tr->info.track_media_kind == MP4E_track_t::e_private)
            {
                WRITE_4(0); // width
                WRITE_4(0); // height
            } else
            ...
            ATOM(BOX_mdia);
                ...
                ATOM(BOX_minf);
                    ...
                    if (tr->info.track_media_kind == MP4E_track_t::e_audio)
                    {
                    ...
                    if (tr->info.track_media_kind == MP4E_track_t::e_video)
                    {
...
#define WRITE_OD_LEN(size) if (size > 0x7F) do { size -= 0x7F; WRITE_1(0x00ff); } while (size > 0x7F); WRITE_1(size)
                        ...
                        if (tr->info.track_media_kind == MP4E_track_t::e_video && (MP4_OBJECT_TYPE_AVC == tr->info.object_type_indication || MP4_OBJECT_TYPE_HEVC == tr->info.object_type_indication))
                        {
