...
struct Shapex
{
  ...
  void renderBoundingRect(GeomRenderer &geomRenderer) {
    ...
    geomRenderer.strokeColor.set(0.3, 0, 0.1, 1.0);
    geomRenderer.strokeWidth = 1.5;
    geomRenderer.strokeType = 1;
    geomRenderer.texture = NULL;
    geomRenderer.drawRect(boundingRect);
  }

  void renderCurves(GeomRenderer &geomRenderer) {
    geomRenderer.strokeWidth = 1.5;
    geomRenderer.strokeType = 1;
    for(int i=0; i<curves.size(); i++) {
      if(i == activeCurveInd) {
        geomRenderer.strokeColor.set(0.75, 0, 0.4, 1.0);
      }
      else {
        geomRenderer.strokeColor.set(0.5, 0, 0.25, 1.0);
      }
      if(curves[i].size() > 1) {
        for(int p=1; p<curves[i].size(); p++) {
          geomRenderer.drawLine(curves[i].points[p-1], curves[i].points[p]);
        }
      }
      if(curves[i].ring && curves[i].size() > 2) {
        geomRenderer.drawLine(curves[i].points[0], curves[i].points[curves[i].size()-1]);
      }
    }
  }
  ...
  void onMousePressed(Events &events) {
    ...
    /*if(curves.size() > 0) {
      for(int i=0; i<curves[0].size(); i++) {
        printf("%.2f, %.2f\n", curves[0].points[i].x, curves[0].points[i].y);
      }
    }*/
    for(int c=0; c<curves.size(); c++) {
      if(c == 0) printf("polygon, num vertices %d\n", curves[c].size());
      else printf("hole, num vertices %d\n", curves[c].size());
    }
  }
  ...
  void onMouseMotion(Events &events, Camera2D *camera = NULL) {
    if(dragMode == 0) {
      if(camera) {
        selectDistance = 10.0 / camera->scale;
      }
      selectPoint(events.mw);
    }
  ...
  void renderPoints(GeomRenderer &geomRenderer) {
    ...
    for(int i=0; i<curves.size(); i++) {
      for(int p=0; p<curves[i].size(); p++) {
        if(i == activeCurveInd && p == activePointInd) {
          geomRenderer.fillColor.set(1.0, 0.3, 0.8, 1.0);
        }
        else {
          geomRenderer.fillColor.set(0.3, 0.0, 0.1, 1.0);
        }
        geomRenderer.drawRect(6, 6, curves[i].points[p]);
      }
    }
  }
  void renderCurvesSmooth(GeomRenderer &geomRenderer) {
    geomRenderer.strokeWidth = 1.5;
    geomRenderer.strokeType = 1;

    for(int i=0; i<curves.size(); i++) {
      if(i == activeCurveInd) {
        geomRenderer.strokeColor.set(0.75, 0.0, 0.4, 1.0);
      }
      else {
        geomRenderer.strokeColor.set(0.5, 0.0, 0.2, 1.0);
      }
      int n = curves[i].size();
      if(n > 1) {
