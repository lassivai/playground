...
struct DrumPad : public Instrument
{
  ...
  struct Pad : public HierarchicalTextFileParser {
    ...
    void init(int sampleRate, int framesPerBuffer) {
      instrument = new Instrument(sampleRate, framesPerBuffer);
    }
  ...
  DrumPad(int sampleRate, int framesPerBuffer) {
    this->sampleRate = sampleRate;
    this->framesPerBuffer = framesPerBuffer;
    instrumentType = InstrumentType::CompositePads;
    
    for(int i=0; i<maxNumPads; i++) {
      pad[i].init(sampleRate, framesPerBuffer);
    }
  }
